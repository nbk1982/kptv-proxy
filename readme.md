...
4. **Quality variants** — with `qualityDedupe` on, a channel the provider carries in several qualities is imported only in its best one: `A&E HD` is dropped because `A&E FHD` survived every earlier stage, while a channel that exists only in HD is untouched. It runs last, on the kept streams only, so an FHD feed that a rule dropped never costs the HD one. Variants are recognised by a quality marker in the name; with the marker removed, the names must be identical (case and spacing aside) and of the same content type. The markers come from `qualityTiers`, best first, one pattern per tier, each matched as a whole word and case-insensitively; empty means the defaults `4k|uhd|2160p`, `fhd|full ?hd|1080p`, `hd|720p`, `sd|480p`. The preview marks these as `Dropped · lower quality`.
